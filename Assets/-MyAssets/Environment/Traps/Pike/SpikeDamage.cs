...
public class SpikeDamage : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (other.CompareTag("Player"))
        {
            if (!IsDownRise)
            {
                other.GetComponent<KamHealth>().GetDamage(damage, 7);        //kam hasar alsýn ve yavaþlasýn;
                if (a == 0) slash1.Play();
                else slash2.Play();
            }
            else if (other.GetComponent<Rigidbody2D>().velocity.y < -0.05f)
            {
                other.GetComponent<KamHealth>().GetDamage(damage * 0.65f, 7);        //kam hasar alsýn ve yavaþlasýn;
                if (a == 0) slash1.Play();
                else slash2.Play();
            }
